...
<?php 
  function cards_principal(){
    ...
    if ($cat >= 1){
      ...
      $sql = "SELECT * FROM postagens WHERE cat = :cat LIMIT :qtd OFFSET :ini";
      $stmt = $conexao->prepare($sql);
      $stmt->bindValue(':cat', $cat, PDO::PARAM_INT);
    }else{
      //query para buscar os posts
      $sql = "SELECT * FROM postagens LIMIT :qtd OFFSET :ini";
      $stmt = $conexao->prepare($sql);
    }
    ...
    if($stmt->execute()) {
        ...
        foreach ($resultado as $campo) {
          //campos da tabela que vão ser incluidos no Card
            exibir_card($campo['titulo'], $campo['conteudo']);
        }
        echo "</div>";
        echo "</div>";
    } else {
